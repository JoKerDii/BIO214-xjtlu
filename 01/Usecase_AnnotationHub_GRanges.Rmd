...
Usecase_AnnotationHub_Granges_Note
---
...
We are going to use `Biocpkg("AnnotationHub")` and `Biocpkg("GenomicRanges")` to access ENCODE data on the H3K4me3 histone modification in a specific cell line.  This histone modification is believed to mark active promoters, and we are going to attempt to verify this statement.  This involves

1. Getting the ENCODE histone data using `Biocpkg("AnnotationHub")`.
2. Getting promoters using `Biocpkg("AnnotationHub")`.
3. Comparing the histone data and promoters using `findOverlaps` in `Biocpkg("GenomicRanges")`.

## Getting the data

First use `Biocpkg("AnnotationHub")` to get data on homo sapiens.
...
Next search for the cell line we are interested in: **H3K4me3** and **Gm12878**
...
Use `title` to refer to the different resources is more robust over time.

The distribution of peak widths:
```{r ahub_summary}
summary(width(gr1))
table(width(gr1)) # almost all peaks have a width of 150bp. We will stick with `gr1`.
summary(width(gr2)) # much more variable
```


To get some promoter coordinates, one way is to do a quick lookup for RefSeq in my annotation hub.
...
We probably need the thing called `RefSeq Genes` and not `Other RefSeq` but why are there multiple resources with the same name? 

Those are the same resources, but in different genome builds.
...
The `table(table())` is a great way to get a quick tabular summary of occurrences with the same name. Almost all genes have a single transcript in Refseq.

Notice that each transcript is a separate range. Therefore, each transcript is represented as the "outer" coordinates of the gene; the ranges does not exclude introns.  Because we got this from UCSC I happen to know that the `$blocks` metadata really contains the coordinates of the different exons. 
...
The default in this function is to use 2kb upstream and 200bp downstream of the start site.  Let's keep this.

Compute which promoters have a H3K4me3 peak in them:
```{r findOverlaps}
...
The rule of thumb is that any cell type has at most 50% of genes expressed, which fits well with these numbers.  We also see that there are many H3K4me3 peaks which do not lie in a genic promoter. This is actually expected.

The overlap is non-empty, but is it interesting or "significant"?  
...
# (result is in megabases; the human genome is 3000 megabases)
```

So overlap is highly unlikely happen purely by chance. 

Let us calculate the overlap in megabases:
```{r size}
sum(width(intersect(gr1, promoters))) / 10^6
```
overlap is empty is because the answer is strand.  We need to ignore the strand when we do this calculation:
```{r size2}
sum(width(intersect(gr1, promoters, ignore.strand = TRUE))) / 10^6
```

This is also affected by strand when there is a promoter on each strand which overlaps.Strand can be troublesome.
